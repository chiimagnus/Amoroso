...
| `HappyPianistAVP/Views/Library/LibraryPracticeProgressOrnamentView.swift` | 以单一内部 `ScrollView` 和系统材质只读展示 loading、首次练习邀请、当前概览、重建提示与 unavailable；包含分段小节进度、恢复位置、近期重点和 Reduce Motion 动画降级，无配置控件或练习按钮。 |
