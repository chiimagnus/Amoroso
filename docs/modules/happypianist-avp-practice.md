...
| `PracticeRoundConfigurationController` | 待生效配置与当前轮配置。 |
...
`HappyPianistAVP/Services/Practice/` 按练习业务链路组织，而不是按底层框架拆散：

| 目录 | 责任 |
| --- | --- |
| `Preparation/` | MusicXML 到 `PreparedPractice`、`PracticeStep` 和准备失败的完整投影管线。 |
| `Session/` | 轮次配置、session 生命周期与 session recorder。 |
| `Input/` | 音频、MIDI、手部输入适配为统一的 `PerformanceObservation`；`Input/AudioRecognition/` 是麦克风目标识别的完整实现。 |
| `Matching/` | 当前 step、和弦、手部 gate 与 attempt 判定。 |
| `Playback/` | `ScorePerformancePlan` 的声音输出、transport、sequence、手动回放和 reset。 |
| `Autoplay/` | 自动播放 timeline 与时间游标。 |
| `Navigation/` | 小节索引与 step 导航。 |
| `Alignment/` | 用户演奏与谱面事件的运行期对齐。 |
| `Assessment/` | 按输入能力生成运行期 assessment。 |
| `Progress/` | attempt reduction、JSON progress repository、checkpoint 与恢复。 |
| `Guidance/` / `Feedback/` | coaching decision、hotspot、next action 与反馈策略。 |
| `Guides/` | 琴键高亮、五线谱布局与 viewport。 |
| `AI/` | 即兴后端、回合控制、CoreML、规则引擎与 AI 播放；`AI/Networking/` 只放 AI 后端的 Bonjour/HTTP/WebSocket 传输。 |
| `Settings/` | 练习声音路由与 session 设置。 |

仍留在 `Services/` 顶层的目录是跨业务或平台边界：`Library/` 负责曲库事务，`MusicXML/` 负责曲谱格式与准备管线，`Recording/` 负责 take 生命周期，`MIDI/` 负责通用 CoreMIDI 收发与协议工具，`PianoMode/Bluetooth/`、`PianoMode/RealPiano/`、`PianoMode/VirtualPiano/`、`HandInput/` 与 `ARSession/` 负责具体设备/空间适配。共享的 `AudioOutputVolumeSettings.swift` 位于 `Library/`，因为它同时服务曲库试听和练习播放；`Infrastructure/` 下统一放置 `Concurrency/`、`Diagnostics/`、`Time/` 与 `Utilities/` 等横切基础设施。
...
- `wrong`、`missing`、`incomplete`、`unknown` 类型化结果
