...
仍留在 `Services/` 顶层的目录是跨业务或平台边界：`Library/` 负责曲库事务，`MusicXML/` 负责曲谱格式与准备管线，`Recording/` 负责 take 生命周期，`MIDI/` 负责通用 CoreMIDI 收发与协议工具，`PianoMode/Bluetooth/`、`PianoMode/RealPiano/`、`PianoMode/VirtualPiano/`、`HandInput/`、`ARSession/` 与 `Immersive/` 负责具体设备/空间适配。`Audio/AudioOutputVolumeSettings.swift` 是曲库试听和练习播放共用的音量设置，因此不归入单一业务模块；`Infrastructure/` 下统一放置 `Concurrency/`、`Diagnostics/`、`Time/` 与 `Utilities/` 等横切基础设施。
