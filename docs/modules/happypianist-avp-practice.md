...
曲库不编辑练习配置。练习窗口准备完成后恢复 exact song UUID + score revision 对应的 active configuration 与位置；无历史进度使用整首、双手、100%、不循环。本轮开始后 active configuration 不可变，练习窗口内的 pending 修改只影响下一轮。
...
-> 无效 passage/resume 时安装当前曲谱整首配置并立即 checkpoint 修复
-> restore step index
-> remain ready/paused
```
