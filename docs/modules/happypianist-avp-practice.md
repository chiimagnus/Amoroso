...
`PracticePlaybackControlService` 从 `ScorePerformancePlan` 构建当前片段的 playback timeline。range/seek 起点由 `PerformanceRangeStateResolver` 恢复 controller、held notes 与 pedal-latched notes，`PerformanceTransportReducer` 以 event identity 处理复调同音、重击和 stale note-off；sequence builder 不再从旧 timeline 反推 controller 状态。踏板事件采用半开区间，不包含片段上界事件，range 结束会显式复位非零 controller。

手动“示范本节”按选定 step range 裁出同一条 plan timeline，保留 velocity、tempo、pedal、grace、arpeggio 与 fermata。当前音提示是独立的短 pitch preview，不是 reference playback。stop、seek、loop、播放错误、音频中断与 route change 都执行 reducer 生成的有序 reset commands，AVAudio/CoreMIDI 层只负责发送命令；`load`/`play` 只机械切换已显式 reset 的序列，不隐式追加第二组命令。

Transport diagnostics 只记录 retrigger、stale/orphan off、range reconstruction、reset reason 与命令数量等 system-only 聚合值，不记录 event identity、逐音序列或原始曲谱。

Plan/range/reducer/timeline 的全量归约使用 Sendable 快照在 MainActor 外完成；ViewModel 以可取消 task 和 generation 安装最新结果，主 Actor 只更新播放状态与 UI 进度。
...
曲库只登记 request；练习根视图在 active scene 激活它，只有 ready 才挂载 `PracticeStepView`。scene inactive 取消准备并 flush，但保留 request。显式返回由根视图协调 generation 失效、full leave、progress flush、session recorder 终结、immersive close/recover 与返回曲库；所有可失败持久化完成后才执行无 IO 的 prepared-song 内存提交。意外消失走独立 best-effort close，不触发返回曲库导航；child view 不维护第二套 leave。
