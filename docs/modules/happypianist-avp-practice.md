...
matcher 只返回 reducer 需要的 typed outcome。当前 step matcher 适合找键和和弦完成练习，不构成连续演奏 alignment 或专业节奏、力度、踏板评价。
...
`PracticeAttemptReducer` 把 step attempt 聚合为 source-measure facts：尝试/成功次数、streak、stable 状态、本轮最后一次 typed issue 和 resume point。
...
反馈不保存到 JSON。进入后台、换曲、restart、窗口关闭和 immersive dismiss 会立即失效 presentation。
...
麦克风 target detection 不具备可靠逐音 release/velocity，不进入 MIDI take。take 可用于回放与后续评价输入，但当前没有 score-performance alignment，不能直接当作专业评分结果。
...
- feedback generation 和 lifecycle cleanup
