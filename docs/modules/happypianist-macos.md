# Module: HappyPianist macOS

`HappyPianist/` 是 macOS MIDI recorder。当前功能边界是监听、录制、导入与回放；它不包含 MIDI mapping、键盘注入或 AVP 的网络后端 client。

## 入口

| 入口 | 说明 |
| --- | --- |
| `HappyPianist/HappyPianistApp.swift` | 创建 SwiftData container、MIDI services、playback services 与 `HappyPianistViewModel`。 |
| `HappyPianist/ContentView.swift` | `MainWindowView`，承载 recorder panel。 |
| `HappyPianist/Views/Recording/RecorderPanelView.swift` | 录制、导入、take 列表、回放输出与 piano roll UI。 |
| `HappyPianist/ViewModels/HappyPianistViewModel.swift` | 状态编排与用户命令。 |
...
`HappyPianistViewModel` 管理以下用户可见状态：
...
```bash
xcodebuild test -project HappyPianist.xcodeproj -scheme HappyPianist -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO
```
