...
| Simulator 集成 | `piano-performance-validation.md` 的 Simulator 覆盖、完整 `xcodebuild test` | `pending evidence` | 对应提交的完整 Simulator run log 通过。 |
...
| Simulator 生命周期 | `piano-performance-validation.md` 的 Simulator 覆盖边界 | `pending evidence` | 对应提交的完整 Simulator run log 通过。 |
...
| 教师有效性 | `piano-performance-validation.md` 的 assessment section v1、rubric `performance-assessment-v2` | `pending evidence` | 独立教师标注完成；按 pitch/timing/duration/dynamics/voicing/pedal 报告 agreement、precision、recall、correlation 和 unknown handling。 |
...
| Simulator 边界 | `piano-performance-validation.md` 的 Simulator 覆盖边界 | `pending evidence` | Simulator 只验证状态机、calibration version、unknown/insufficient 和恢复，且有完整 run log。 |
...
| assessment 有效性 | `piano-performance-validation.md` 的 assessment section v1、rubric `performance-assessment-v2` | `pending evidence` | 触发指导的 assessment 已通过对应 CG-002 的教师标注 gate。 |
| 教学有效性 | `piano-performance-validation.md` 的 coaching section v1 | `pending evidence` | 预登记的 before/action/dose/after/control/transfer/adverse-effect 研究完成并保留聚合结果。 |
