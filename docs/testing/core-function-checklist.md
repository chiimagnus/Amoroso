...
3. 设置 60%–80% 速度。
4. 完成至少一轮练习。
5. 若完成摘要出现练习建议，分别执行一次建议和一次“跳过指导并继续”。
...
- [ ] 左手模式不会被右手专属 step 卡死
- [ ] 达到连续成功目标后能结束本轮
- [ ] 完成摘要最多显示一个动作；证据不足时只请求再次演奏，不显示确定性错误结论
- [ ] 接受建议后应用其小节范围，速度只会降低到建议上限而不会被意外提高
- [ ] 手别、指法、声部或参考来源存在时以中性文字展示，VoiceOver 可读
- [ ] 从完成摘要跳过指导后能开始下一轮，旧决定不会再次驱动当前轮
- [ ] 开启循环后，达标时不会无限循环
...
Simulator 自动化入口与未覆盖边界见 [钢琴演奏专业化验证手册](piano-performance-validation.md)；只有附有该次 `xcodebuild test` 运行日志的记录才能标为通过。

- [ ] MusicXML 与演奏事件快照稳定，差异已人工确认
- [ ] fixture manifest 无重复、缺失或未登记文件
- [ ] 已知偏差 manifest 覆盖全部审查编号，未把错误固化为正确快照
- [ ] 输入重放不依赖真实时间、硬件或网络
- [ ] visionOS `xcodebuild test` 已在实际 destination 运行
- [ ] 真机延迟、抖动、漏触发、卡音与恢复证据已独立记录
- [ ] 需要专业承诺时，钢琴家盲听与教学有效性证据已完成
