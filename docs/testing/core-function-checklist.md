...
分别使用系统水平滚动、点按中心/邻项和 VoiceOver adjustable action 执行：

1. 在 min/ideal/max 窗口宽度下滚动到首项、末项和中间项，并在滚动途中跨过至少三首。
2. 点按中心唱片切换试听，再点按完整可见和边缘裁切的非中心唱片。
3. resize 后确认当前 selection 重新居中；随后立即点击“开始练习”并离开曲库再进入。
4. 分别执行横向、上拽、下拽和对角线手势。

检查：

- [ ] 系统负责惯性与 view-aligned 吸附，首尾唱片均可居中且没有左右箭头
- [ ] 窗口宽度自然决定完整与裁切的可见唱片数量；中心最大最清晰，邻项随距离缩小、变淡、降饱和
- [ ] 滚动途中不停止试听或提交中间 selection，落定后只发布最终中心曲目
- [ ] 点按非中心唱片后立即选中并居中；点按中心唱片只切换试听
- [ ] 每次最终选择后唱片、曲名和来源立即更新，不等待磁盘写入
- [ ] 上拽导入、下拽 hold-delete、VoiceOver adjustable action 与对角线输入不回归
- [ ] 只有当前内存 song ID 在练习窗口进入 ready 或 failure
- [ ] 离开曲库后不继续保存或发布旧选择结果
- [ ] 开启 Reduce Motion 后仍保留静态尺寸与明暗层级，并满足上述状态与取消规则
