...
-> MusicXMLPracticePartSelector + filtering(toLogicalInstrument:)
-> explicit MusicXMLOrderSelection + optional MusicXMLStructureExpander
-> MusicXMLHandRouter assignments (不改写 staff / voice)
-> shared ScoreTimingScheduleBuilder
-> tempo / pedal / fermata / attribute timelines
-> ScorePerformancePlanBuilder
-> PracticeStepBuilder + PianoHighlightGuideBuilderService projections
-> ScoreNotationProjection (plan + source score)
-> PreparedPractice
```
...
- source / prepared score、logical instrument、structural part、order selection 与 hand assignments

tempo、pedal 与 fermata 只在 preparation 内参与 plan 构建，不再作为平行声音事实进入 session；tempo 查询表由 plan 自动派生。旧 `MusicXMLNoteSpanBuilder` 路径已删除。

### 自动播放

```text
PreparedPractice.performancePlan
-> AutoplayPerformanceTimeline
-> PracticeSequencerSequenceBuilder / CoreMIDI scheduler
-> local sampler or external MIDI
```

highlight guides 与 steps 只为 timeline 补充 UI 导航事件和范围边界；音高、力度、时值、tempo 与控制器始终来自 plan。手动逐步发声同样按当前 step 的 tick 区间选择 plan 事件，不读取 step 内的声音字段。
...
#### NOTATION-001：当前五线谱是 source-backed 练习投影，不是完整原谱渲染

**严重度：P0（若对外称为权威谱面）｜证据：代码确认**

`ScoreNotationProjection` 已同时携带 source written facts 与 plan performed occurrence；layout 不再从 highlight 或 performed duration 反推权威时值与 accidental。当前 renderer 仍按 MIDI 定位谱表高度，且 accidental 表现只覆盖有限的 source alter，未完整呈现降号、重升降、courtesy/cautionary 与调号上下文。

影响：source spelling 已保留但尚未被完整渲染；Db/C# 的谱表位置、临时记号取消、双升降与调号上下文仍可能显示错误。
...
note value 读取 `ScoreNotationProjection.SourceNote.writtenDurationTicks`；staccato、release 与 fermata 改写的 `performedOffTick` 只用于演奏 occurrence，不再改变 written note value。

剩余边界：tuplet、beam、复杂附点与符号组合仍需完整 source notation contract 和 renderer 支持。
...
| 优先级 | 必须先解决 |
| --- | --- |
| P0 | 规范 dynamics、grace、MIDI 数据保留、输入能力/时钟、palm 触键、相邻半音容差、score alignment 边界、产品宣称 |
| P1 | direction offset exporter corpus、arpeggio、seek state、fermata、连续踏板、CoreMIDI timestamp、音频恢复、完整记谱渲染、录制证据 |
...
- `HappyPianistAVP/Models/Practice/ScorePerformancePlan.swift`
- `HappyPianistAVP/Services/Practice/Performance/ScorePerformancePlanBuilder.swift`
...
- `HappyPianistAVP/Services/Practice/Playback/PracticeManualReplayService.swift`
- `HappyPianistAVP/Services/Practice/Playback/PlaybackSequenceBuilder.swift`
