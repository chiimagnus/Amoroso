...
struct PianoKeyContactTracker {
    ...
    mutating func detect(
        ...
        fingerTips.forEachFinger { fingerID, worldPosition in
            ...
            let threshold = continuesPreviousContact
            ...
            if continuesPreviousContact, let previous {
            ...
            observations.append(
                PianoKeyContactObservation(
                    id: contactID,
                    phase: phase,
                    keyCandidate: .exact(region.midiNote),
                    timestamp: timestamp,
                    confidence: 1,
                    worldPosition: worldPosition,
        ...
        for (fingerID, previous) in previousContacts where currentContacts[fingerID]?.id != previous.id {
            ...
            let endedLocalPosition = Self.transformPoint(keyboardFromWorld, endedWorldPosition)
            observations.append(
                PianoKeyContactObservation(
                    id: previous.id,
                    phase: .ended,
                    keyCandidate: .exact(previous.midiNote),
                    timestamp: timestamp,
                    confidence: currentWorldPosition == nil ? 0 : 1,
                    worldPosition: endedWorldPosition,
                    planeDistanceMeters: endedLocalPosition.y - previous.surfaceLocalY,
                    normalVelocityMetersPerSecond: nil,
